...
const DoctorInformation = () => {
  ...
  const fetchAppointments = async (doctorId) => {
    try {
      const response = await axios.get(`https://localhost:800/api/appointments/doctor/${doctorId}`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.put(`https://localhost:800/api/doctor/${doctor._id}`, doctor, {
