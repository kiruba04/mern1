...
const UserInformation = () => {
  ...
  const fetchAppointments = async (userId) => {
    try {
      const response = await axios.get(`https://localhost:800/api/appointments/${userId}`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.put(`https://localhost:800/api/user/${user._id}`, user, {
  ...
  const deleteappointment = async (appointmentId) => {
    try {
      await axios.delete(`https://localhost:800/api/appointment/delete/${appointmentId}`);
